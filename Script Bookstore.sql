...
#    constraint fk_cpf_autor foreign key(cpf) references PESSOA(cpf) on update cascade on delete restrict
);

CREATE TABLE IF NOT EXISTS autoria (
	id_autor integer(8),
    cod_produto bigint(10),
    primary key(id_autor,cod_produto),
    constraint fk_autoria_autor foreign key(id_autor) references AUTOR(id_autor) on update cascade on delete cascade,
    constraint fk_autoria_livro foreign key(cod_produto) references LIVRO(cod_produto) on update cascade on delete cascade
);
...
CREATE TABLE IF NOT EXISTS encomenda (
	id_encomenda bigint(10) not null,
    dt_pedido date,
...
CREATE TABLE IF NOT EXISTS produto_ref (
	cod_produto bigint(10) not null,
...
CREATE TABLE IF NOT EXISTS item_estoque (
	cod_produto bigint(10) not null,
    seq_SK bigint(10) not null,
    em_prateleira boolean,
    preco_venda double not null,
    qtd_minima integer(5),
    dt_entrada date,
    qtd_atual integer(5),
    id_estoque bigint(10) unique,
    serial_prateleira bigint(10) unique,
...
CREATE TABLE IF NOT EXISTS prateleira (
	serial_prateleira bigint(10) not null,
...
CREATE TABLE IF NOT EXISTS nota_fiscal (
	codigoNF bigint(10) not null,
    dt_emissao date not null,
    serie bigint(10),
    vl_total double not null,
    cod_compra bigint(10) not null unique,
...
CREATE TABLE IF NOT EXISTS pagamento (
	cod_pag bigint(10) not null,
    vl_pago double not null,
    codigoNF bigint(10) unique,
    primary key(cod_pag),
...
CREATE TABLE IF NOT EXISTS genero_livro (
	cod_livro bigint(10) not null unique,
    cod_genero integer(5) not null unique,
    primary key(cod_livro, cod_genero),
...
CREATE TABLE IF NOT EXISTS organiza (
    ...
    serial_prateleira bigint(10) not null unique,
    dt_inicio date,
    dt_fim date,
    dt_ultima_reposicao date,
    primary key(cpf_auxiliar, serial_prateleira),
...
CREATE TABLE IF NOT EXISTS inscricao (
    ...
    dt_inscricao date,
    compareceu boolean,
	primary key(cod_evento, cpf_cliente),
...
CREATE TABLE IF NOT EXISTS comissao_funcionario (
    ...
    primary key(id_comissao, cpf_vendedor),
